...
export function GamesHub() {
  ...
  const openModal = (subject: string) => { setSelectedGame(subject); setModalOpen(true); };
  const closeModal = () => { setModalOpen(false); setSelectedGame(null); };
    ...
    <div className="w-full max-w-6xl mx-auto pb-20">
      ...
      <section className="text-center py-10 md:py-16 relative overflow-hidden mb-8">
        <div className="relative z-10">
          <h1 className="font-['Press_Start_2P'] text-3xl md:text-5xl lg:text-6xl text-[#00ff88] mb-4 drop-shadow-[0_0_20px_rgba(0,255,136,0.6)]">
      ...
      <section id="materias" className="mb-16">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="h-1 flex-1 bg-[#00ff88]/20 rounded-full" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <GameCard title="MATEMATICAS" desc="Numeros & Logica"     coins={250} time="15m" color="#4169E1" icon={<Calculator size={32} />}  onClick={() => openModal("Matematicas")} />
          <GameCard title="CIENCIAS"    desc="Quimica & Fisica"     coins={200} time="12m" color="#228B22" icon={<FlaskConical size={32} />} onClick={() => openModal("Ciencias")} />
          <GameCard title="LENGUA"      desc="Gramatica & Letras"   coins={180} time="10m" color="#DAA520" icon={<BookOpen size={32} />}    onClick={() => openModal("Lengua")} />
          <GameCard title="SOCIALES"    desc="Historia & Geografia" coins={220} time="13m" color="#DC143C" icon={<Globe2 size={32} />}      onClick={() => openModal("Sociales")} />
        </div>
      </section>
      ...
      <section className="mb-16">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="h-1 flex-1 bg-[#00d9ff]/20 rounded-full" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <ModeCard badge="POPULAR" title="NORMAL"       desc="Sin presion de tiempo."       icon={<Brain size={28} />}  color="#00d9ff" />
          <ModeCard badge="HOT"     title="COMPETENCIA"  desc="Ranking mundial en vivo."     icon={<Trophy size={28} />} color="#ff1b8d" isHot />
          <ModeCard badge="NEW"     title="MULTIJUGADOR" desc="Desafia a tus amigos."        icon={<Users size={28} />}  color="#00ff88" isNew />
        </div>
      </section>
      ...
      <section className="mb-16">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="h-1 flex-1 bg-[#ff1b8d]/20 rounded-full" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <DifficultyCard title="FACIL"   icon={<Shield size={32} />} level={1} color="#00ff88" />
          <DifficultyCard title="MEDIO"   icon={<Rocket size={32} />} level={2} color="#ffd700" />
          <DifficultyCard title="DIFICIL" icon={<Sword size={32} />}  level={3} color="#DC143C" />
        </div>
      </section>
          ...
          <div className="space-y-3">
            {LEADERBOARD.map((player, index) => (
              <div key={index}
                ...
                <div className="w-10 text-center font-['Press_Start_2P'] text-lg">{index + 1}</div>
      ...
      {/* Modal — sin AnimatePresence */}
      {modalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0f1425] border-4 border-[#00d9ff] w-full max-w-md rounded-xl shadow-[0_0_40px_rgba(0,217,255,0.5)] overflow-hidden">
...
const GameCard = ({ title, desc, coins, time, color, icon, onClick }: any) => (
  <motion.div whileHover={{ y: -5 }} onClick={onClick}
    className="group relative bg-[#0f1425] border-2 border-transparent hover:border-[var(--color)] rounded-xl p-6 cursor-pointer overflow-hidden transition-all shadow-lg"
    style={{ '--color': color } as any}>
    <div className="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
      {React.cloneElement(icon, { size: 60 })}
    </div>
    <div className="relative z-10 flex flex-col h-full">
      <div className="w-12 h-12 rounded-lg flex items-center justify-center mb-4 text-white shadow-lg" style={{ backgroundColor: color }}>
        {icon}
      </div>
...
const ModeCard = ({ badge, title, desc, icon, color, isHot, isNew }: any) => (
  <motion.div whileHover={{ scale: 1.02 }}
    className="relative bg-[#1a1f35] border border-white/10 p-5 rounded-xl hover:bg-[#0f1425] transition-all cursor-pointer group"
    style={{ '--color': color } as any}>
    <div className={cn(
