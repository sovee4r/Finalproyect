...
export function QuizLengua4() {
  ...
  useEffect(() => { okCountRef.current  = okCount;  }, [okCount]);
  useEffect(() => { badCountRef.current = badCount; }, [badCount]);

  // Auto-completar nombre con usuario logueado
  useEffect(() => {
    if (user?.nombre) {
      setPlayerName(user.nombre);
    }
  }, [user]);
  ...
  useEffect(()=>{
    if(estaJugandoMulti) multiMusic.start();
    else multiMusic.stop();
  },[estaJugandoMulti]);
  ...
  useEffect(()=>{ if(multiState.tiempoAgotado){setMultiAnswered(true);} },[multiState.tiempoAgotado]);
  useEffect(()=>{ setMultiAnswered(false); setMultiSelAns(null); },[multiState.preguntaIdx]);
  ...
  function initGame(qs:Pregunta[]){
    ...
    scoreRef.current=0; okCountRef.current=0; badCountRef.current=0;
  ...
  function onTimeout(qs:Pregunta[],qi:number){
    ...
    setBadCount(p=>p+1); badCountRef.current++;
  ...
  function handleAnswer(letter:string){
    ...
    if(letter===correct){
      ...
      setOkCount(p=>p+1); okCountRef.current++;
  ...
  function nextQ(qs:Pregunta[],qi:number){
    ...
    if(next>=qs.length){
      music.stop();
      fetch("https://finalproyect-production-3837.up.railway.app/api/resultados_juegos", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          materia:     "lengua",
          grado:       4,
          puntos:      scoreRef.current,
          correctas:   okCountRef.current,
          incorrectas: badCountRef.current,
          tiempo_seg:  0,
          modo:        "solo",
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen text-white relative">
      ...
      {estaEnLobby && multiState.sala && (
        <MultiLobby
          sala={multiState.sala} esHost={esHost} nombrePropio={playerName}
      ...
      {estaEnResultadosMulti && (
        <MultiRanking
          ...
          onJugarDeNuevo={handleSalirMulti} onSalir={handleSalirMulti}
        />
      )}

      {screen==="config" && !estaEnLobby && !estaJugandoMulti && !estaEnResultadosMulti && !(gameMode==="multi" && multiState.estado==="error") && (
        <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} className="w-full max-w-xl px-4 py-8">
          ...
          <motion.button whileHover={{scale:1.02,y:-2}} whileTap={{scale:0.98}}
            ...
            style={{
              background: gameMode==="solo" ? "linear-gradient(135deg,#DAA520,#ff9800)" : "linear-gradient(135deg,#a78bfa,#7c3aed)",
              boxShadow:  gameMode==="solo" ? "0 4px 24px rgba(218,165,32,0.4)"         : "0 4px 24px rgba(167,139,250,0.35)",
            }}>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="w-full max-w-lg px-4 py-10 text-center">
          ...
          <div className="rounded-2xl border-2 border-[#ffd700]/20 p-6 mb-8"
            ...
            <div className="flex justify-center gap-12">
              ...
              <div><div className="font-['Press_Start_2P'] text-2xl text-[#ff9800]">+{score}</div><div className="text-sm font-bold text-gray-500 mt-2">XP</div></div>
            </div>
          </div>
          <motion.button whileHover={{scale:1.02,y:-2}} whileTap={{scale:0.98}}
...
interface GameScreenProps {
  q: Pregunta; curQ: number; totalQ: number;
  ...
  onSalirMulti?: ()=>void;
  music?: { muted: boolean; toggleMute: () => void };
}
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            {isMulti && music && (
              <button onClick={music.toggleMute}
                ...
                {music.muted ? <Volume2 size={15}/> : <VolumeX size={15}/>}
              </button>
            )}
            {!isMulti && (
              <>
